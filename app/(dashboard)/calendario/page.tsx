...
useEffect(() => {
  ...
  // Lógica para gerar os quadrados do calendário
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* LADO ESQUERDO: Grelha do Calendário[cite: 3] */}
          ...
          <div className="grid grid-cols-7 gap-px bg-gray-100">
            ...
            {diasNoMes.map((dia: Date) => (
              <div key={dia.toString()} className="bg-white min-h-[120px] p-2 border-t border-gray-50">
                ...
                {/* Mostrar mini-etiquetas de eventos aqui[cite: 3] */}
        ...
        {/* LADO DIREITO: Lista de Próximos Eventos[cite: 3] */}
        <div className="lg:col-span-4 space-y-6">
          <h3 className="text-xl font-black text-[#002d5e]">Eventos de {format(currentDate, 'MMMM', { locale: pt })}</h3>
          <div className="space-y-4">
            {eventos.slice(0, 4).map(e => (
              <div key={e.id} className="flex gap-4 bg-white p-4 rounded-2xl border border-gray-50 shadow-sm">
                <div className="w-12 h-12 bg-blue-50 rounded-xl flex flex-col items-center justify-center text-[#002d5e]">
                  <span className="text-[10px] font-black uppercase">{format(new Date(e.data_inicio), 'MMM', { locale: pt })}</span>
                  <span className="text-lg font-black">{format(new Date(e.data_inicio), 'dd')}</span>
                </div>
                <div className="flex-1">
                  <h4 className="font-bold text-sm text-gray-900">{e.titulo}</h4>
                  <div className="flex items-center gap-2 text-[10px] text-gray-400 mt-1"><MapPin size={12}/> {e.local}</div>
                  <div className="flex items-center gap-2 text-[10px] text-gray-400"><Clock size={12}/> {format(new Date(e.data_inicio), 'HH:mm')}</div>
                </div>
              </div>
            ))}
          </div>
