  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (user) {
        ...
        // Simular carregamento da mensagem da BD (podes criar uma tabela 'configuracoes' para isto)
        setMensagemGov(perfil?.bio || "O Rotary é a oportunidade de transformar o mundo através do serviço.")
      }

      const { data: todosMembros } = await supabase.from('perfis').select('*').order('primeiro_nome')
      if (todosMembros) setEquipa(todosMembros)
      setLoading(false)
    }
    loadData()
  }, [])
    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-20">
      ...
      {viewMode === 'admin' ? (
        <div className="space-y-10 animate-in fade-in duration-500">
          ...
          {/* 1. MENSAGEM DO GOVERNADOR (ESTILO VISTA NORMAL) */}
          <section className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row items-center gap-10">
              <div className="relative">
                ...
                <div className="absolute -bottom-2 -right-2 bg-[#fca311] p-1.5 rounded-full text-white"><Star size={14} fill="currentColor"/></div>
              </div>
              
              <div className="flex-1 w-full space-y-4">
                <div>
                  <span className="text-[10px] font-black uppercase text-[#fca311] tracking-widest">Governador Distrital 2024-25</span>
                  <h2 className="text-2xl font-black text-[#004a99]">{governador?.primeiro_nome} {governador?.apelido}</h2>
                </div>
                ...
                <div className="relative">
                  <textarea 
                    ...
                    className="w-full bg-gray-50 border-l-4 border-orange-200 p-4 italic text-gray-600 text-sm rounded-r-xl focus:ring-0 focus:border-orange-400 outline-none resize-none"
                    rows={3}
                  />
                  <button 
                    className="absolute bottom-2 right-2 bg-[#004a99] text-white p-2 rounded-lg hover:bg-blue-700 transition shadow-md"
                    title="Guardar Mensagem"
                  >
                    <Save size={16} />
                  </button>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div className="lg:col-span-2 bg-white p-8 rounded-[24px] border border-gray-100 shadow-sm">
               ...
               <div className="space-y-4">
                  <input className="w-full bg-gray-50 border border-gray-100 rounded-xl p-3.5 text-sm outline-none" placeholder="Procurar membro..." />
                  ...
                  <button className="w-full bg-[#004a99] text-white py-4 rounded-xl font-black text-sm flex items-center justify-center gap-3 shadow-lg">Confirmar Atribuição</button>
               </div>
            </div>
            ...
            <div className="lg:col-span-3 bg-white p-8 rounded-[24px] border border-gray-100 shadow-sm">
               ...
               <div className="space-y-3">
                  <div className="p-4 bg-gray-50 rounded-xl flex justify-between items-center">
                    <span className="font-bold text-gray-700 text-sm">Desenvolvimento e Expansão</span>
                ...
                <tbody className="text-sm divide-y divide-gray-50">
                  {equipa.map((m, i) => (
                    <tr key={i} className="hover:bg-gray-50/50 transition">
                      ...
                      <td className="px-8 py-4 text-right"><div className="w-2 h-2 bg-green-500 rounded-full ml-auto"></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-700">
           {/* Vista Pública (A que já tínhamos feito) */}
...
function PublicTeamView({ members, mensagem }: { members: any[], mensagem: string }) {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Restantes membros seriam mapeados aqui */}
    </div>
  )
}